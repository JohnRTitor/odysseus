...
def resolve_parameters(endpoint_url: str, model_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for cand in endpoints:
            if cand.base_url and normalize_base(cand.base_url) == norm_req:
                best_ep = cand
                # Prioritize the endpoint if it actually has parameters for this model
                if cand.model_parameters and f'"{model_id}"' in cand.model_parameters:
                    break
                    
        if best_ep and best_ep.model_parameters:
            try:
                params_dict = json.loads(best_ep.model_parameters)
                if isinstance(params_dict, dict) and model_id in params_dict:
